/* JavaScript para Server Side actividad_04_extra.html */
...
if (numeroPsjs == 0) {
  ...
  infoBoton.style.cursor = "not-allowed";
} else {
  infoBoton.disabled = false;
  infoBoton.style.cursor = "pointer";
}
...
function func_guardaLS(evento) {
  ...
  let tempMasc = document.getElementById("radioMasc").checked; // .checked regresa: true/false
  // si el valor es: true/false se asigna a gnr: masculino/femenino
  let gnr = tempMasc ? "Masculino" : "Femenino"; // if de una linea
  ...
  // reconstruye la lista de personajes cada vez ques e agrega uno
  cargPsj();
...
function cargPsj() {
  ...
  while (selector.options.length > 0) selector.remove(0); // while de una line no requiere {}
  // keys de todos los personajes guardados
  let idsPersonajes = Object.keys(localStorage);
